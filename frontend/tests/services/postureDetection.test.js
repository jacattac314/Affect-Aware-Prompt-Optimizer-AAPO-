import { classifyPosture } from '../../src/services/postureDetection';

// Mock the @mediapipe/pose dynamic import so tests don't need a DOM or WASM.
...
describe('classifyPosture (client-side heuristics)', () => {
  ...
  test('all valid posture labels are strings', () => {
    const cases = [
      buildLandmarks(),                                             // upright
      buildLandmarks({ 0: lm(0.50, 0.42) }),                       // head-down
      buildLandmarks({ 0: lm(0.50, 0.32) }),                       // slouching
      buildLandmarks({ 15: lm(0.65, 0.55), 16: lm(0.35, 0.55) }), // crossed-arms
      buildLandmarks({ 0: lm(0.14, 0.20) }),                        // leaning-forward
    ];
    cases.forEach((lms) => {
